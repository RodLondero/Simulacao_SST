...
def configuraGraficos(figs, tempo, t):
    for fig in figs:
        ...
        x = tempo
        if len(tempo) > 24:
            fig.axes[0].set_xlim([60, t])
            x = np.arange(60, t+60, 60)
            # vetor = range(1, t+60, 60)
        ...
        fig.axes[0].set_xticks(x)
        fig.axes[0].xaxis.set_minor_locator(AutoMinorLocator(2))
        fig.axes[0].yaxis.set_minor_locator(AutoMinorLocator(2))
        fig.axes[0].grid(True)
        fig.axes[0].grid(b=True, which='minor', color='#DCDCDC')

        if len(tempo) > 24:
            fig.canvas.draw()
            labels = [item.get_text() for item in fig.axes[0].get_xticklabels()]
